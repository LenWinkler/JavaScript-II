...
// Problem 1
// Create an array with the runners who donated over $200

let vipRunners = [];

runners.filter(function(item) {
  if (item.donation > 200) {
    vipRunners.push(`${item.first_name} ${item.last_name}`)
  }
});
console.log(vipRunners);

// Problem 2
// List first/last name and email of each runner to create mailing list

mailingList = [];

runners.forEach(function(item) {
  mailingList.push(`Name: ${item.first_name} ${item.last_name}  Email: ${item.email}`)
});
console.log(mailingList);

// Problem 3
//We want to email everyone and thank them for their donations. Generate thank you messages in 'thankYou' array

thankYou = [];

runners.map(function(item) {
  thankYou.push(`Hi ${item.first_name}! We just wanted to say thanks for your donation of ${item.donation} dollars. We couldn't do what we do without the generousity of people like you!`)
});
console.log(thankYou[0]); //console log one so you can see the whole thing
console.log(thankYou); // console log everything. it cuts certain things off